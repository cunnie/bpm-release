	...
	return specs.Spec{
		...
		Linux: &specs.Linux{
			MaskedPaths: []string{
				...
				"/proc/timer_stats",
				"/sys/firmware",
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.out.log", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.err.log", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.out.log.1.gz", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.err.log.1.gz", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.out.log.2.gz", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.err.log.2.gz", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.out.log.3.gz", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.err.log.3.gz", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.out.log.4.gz", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.err.log.4.gz", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.out.log.5.gz", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.err.log.5.gz", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.out.log.6.gz", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.err.log.6.gz", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.out.log.7.gz", bpmCfg.JobName(), bpmCfg.ProcName()),
				fmt.Sprintf("/var/vcap/sys/log/%s/%s.err.log.7.gz", bpmCfg.JobName(), bpmCfg.ProcName()),
			},
